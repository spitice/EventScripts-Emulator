...
class Connection(object):
    ...
    def connect(self):
        ...
        try:
            if self.data is not None:
                data = urllib.parse.urlencode(self.data).encode('ascii')
                self.connection = urllib.request.urlopen(self.url, data)
            else:
    ...
    def reCacheUpdate(self):
        ...
        """
        if self.status == self.__class__.CONNECTED:
            self.cachedUpdateSource = self.connection.read().decode('ascii')
